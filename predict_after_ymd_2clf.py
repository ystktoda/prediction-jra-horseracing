...
if __name__ == '__main__':
    ...
    horse_table_list = t_util1.get_horse_table_list(test_race_keys)
    ...
    for horse_table in horse_table_list:
        horse_table.predict_2clf(t_util1.clf, t_util1.sc, t_util2.clf, t_util2.sc)
        for n in range(train.TOP_N):
            pr = horse_table.get_predict_return_2clf(n + 1)
            if pr is not None:
                df_list[n].append(pr)
        cnt += 1
